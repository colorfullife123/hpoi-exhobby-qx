...
当前版本：**v3.4.0**。自动识别不同手办词条，为每个条目分别保存图库和分页数据。外层详情页只添加相册入口，不插入预览图片网格。EXHOBBY 相册列表使用单独设计的标识封面，进入相册后仍显示各词条自己的图库图片。
...
- 点击相册内入口后继续使用现有的本地虚拟相册 ID，进入原生 EXHOBBY 相册；不会把虚拟 ID 直接发送给 Hpoi 后端。
...
- 在每个属于当前词条的普通 Hpoi 相册第一页顶部也添加 EXHOBBY 入口，原相册图片保持不变。
...
保存并更新远程资源，确认资源成功加载九条重写规则。EXHOBBY 主脚本使用带 `v=3.4.0` 标记的地址，避免继续使用旧脚本缓存；开屏广告清理脚本仍可使用其独立缓存版本。订阅同时引用 `assets/exhobby-cover-v3.2.png`，无需额外保存同名本地脚本。
