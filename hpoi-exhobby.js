// Hpoi + EXHOBBY native album v3.5.4 — Quantumult X
...
(function () {
  ...
  async function notifyVerification(error) {
    ...
    if (!save("verify-notice", { time: now })) {
      log("v3.5.4 notice cooldown could not be saved");
    }
    ...
    if (/^https:\/\/[^\s/?#]+\/[^\s/?#]+\/?$/.test(barkURL)) {
      ...
      log("v3.5.4 Bark delivery failed; falling back to Quantumult X notice");
    }
    ...
    if (confirmed) {
      ...
      if (cookie && !/[\r\n]/.test(cookie) && !/[\r\n]/.test(agent)) {
        if (save("browser-session", { cookie: cookie, agent: agent, time: Date.now() })) {
          ...
          log("v3.5.4 browser session saved; reopen Hpoi");
        } else log("v3.5.4 browser session cache write failed");
      } else log("v3.5.4 gallery loaded, but Cookie header missing; tap More in Safari");
    }
    done();
    return true;
  }
  ...
  function makeExClient() {
    ...
    function htmlInfo(html, stage) {
      ...
      log("v3.5.4 " + stage + " title=" + label(title && title[1]) +
    ...
    function readText(response, stage) {
      ...
      log("v3.5.4 " + stage + " HTTP=" + response.statusCode +
    ...
    async function request(url, method, body, stage, ticket) {
      ...
      for (var hop = 0; hop < 5; hop++) {
        ...
        if ([301, 302, 303, 307, 308].indexOf(status) >= 0) {
          ...
          log("v3.5.4 " + stage + " redirect=" + url.split("?")[0]);
    ...
    function parseFirst(html) {
      ...
      log("v3.5.4 first HTML count=" + rows.length);
      return rows;
    }
    async function bootstrap(ticket) {
      ...
      log("v3.5.4 fresh map.url ready; preview list not used");
    ...
    return {
      page: async function (page, ticket) {
        ...
        log("v3.5.4 page=" + page + " count=" + list.length);
        return list;
      }
    };
  }
  function fetchPage(page, milliseconds) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        finish(new Error("v3.5.4 page=" + page + " timeout; refresh to resume"));
      }, milliseconds);
  ...
  async function loadGallery() {
    ...
    try {
      ...
      if (work.next > 1 && work.first !== firstSignature) {
        work = { version: 30, started: now, next: 1, rows: [], last: "", first: "" };
        log("v3.5.4 first page changed; restarting pagination");
      }
  ...
  async function main() {
    ...
    if (!isResponse) {
      ...
      log("runtime=v3.5.4 phase=request endpoint=" + endpoint);
      ...
      if (v && v.type === "normal") {
        ...
        ctx.remapped = true;
        log("v3.5.4 normal album proxy remapped to native route");
      } else if (v) {
        ...
        ID_FIELDS.forEach(function (k) {
          ...
          if (p[k] && (nativeId === ALBUM || nativeId > PIC_BASE || dedicatedRoute) &&
            ...
            changes[k] = safeId;
            log("v3.5.4 remap request field " + k + " via template ID");
          }
        });
    ...
    if (endpoint === "hobby/album" && ITEM && Number(ctx.p.id) === ITEM &&
      ...
      // v3.5.4: never reserve, proxy, or rewrite any native Hpoi album.
  ...
  main().catch(function (e) {
    log("v3.5.4 " + (e && e.message ? e.message : "operation failed"));
