// Hpoi + EXHOBBY native album v3.7.0 — Quantumult X / Surge / Loon
...
(function () {
  ...
  var PLATFORM = typeof $task !== "undefined" && typeof $prefs !== "undefined"
    ? "quantumult-x" : (typeof $loon !== "undefined" ? "loon" : "surge");
  ...
  function storageRead(key) {
    if (typeof $prefs !== "undefined" && $prefs &&
        typeof $prefs.valueForKey === "function") return $prefs.valueForKey(key);
    if (typeof $persistentStore !== "undefined" && $persistentStore &&
        typeof $persistentStore.read === "function") return $persistentStore.read(key);
    return null;
  }
  ...
  function storageRemove(key) {
    var current = storageRead(key);
    var existed = current != null && current !== "";
    if (typeof $prefs !== "undefined" && $prefs &&
        typeof $prefs.removeValueForKey === "function") {
      var removed = $prefs.removeValueForKey(key);
      return existed && removed !== false;
    }
    if (typeof $persistentStore !== "undefined" && $persistentStore &&
        typeof $persistentStore.write === "function") {
      // Surge/Loon only document string writes. An empty string is read as
      // missing by this runtime and avoids relying on an undocumented null delete.
      var cleared = $persistentStore.write("", key);
      return existed && cleared !== false;
    }
    return false;
  }
  function responseStatus(response) {
    return Number(response && (response.statusCode != null
      ? response.statusCode : response.status));
  }
  function normalizeHTTPResponse(response, data) {
    response = response || {};
    var normalized = {};
    Object.keys(response).forEach(function (key) { normalized[key] = response[key]; });
    normalized.statusCode = responseStatus(response);
    if (normalized.status == null && Number.isFinite(normalized.statusCode)) {
      normalized.status = normalized.statusCode;
    }
    if (data !== undefined) normalized.body = data;
    if (!normalized.headers) normalized.headers = {};
    return normalized;
  }
  function httpFetch(options) {
    ...
    return new Promise(function (resolve, reject) {
      if (typeof $httpClient === "undefined" || !$httpClient) {
        reject(new Error("HTTP client unavailable")); return;
      }
      var method = String(options.method || "GET").toLowerCase();
      var fn = $httpClient[method];
      if (typeof fn !== "function") {
        reject(new Error("unsupported HTTP method")); return;
      }
      var requestOptions = {
        url: options.url,
        headers: options.headers || {}
      };
      if (options.body != null) requestOptions.body = options.body;
      if (options.timeout != null) requestOptions.timeout = options.timeout;
      if (options.opts && options.opts.redirection === false) {
        requestOptions["auto-redirect"] = false;
      }
      if (options.opts && options.opts["auto-cookie"] === false) {
        requestOptions["auto-cookie"] = false;
      }
      fn.call($httpClient, requestOptions, function (error, response, data) {
        if (error) reject(new Error(String(error)));
        else resolve(normalizeHTTPResponse(response, data));
      });
    });
  }
  function postNotification(title, subtitle, message, url) {
    if (typeof $notify === "function") {
      $notify(title, subtitle, message, url ? { "open-url": url } : undefined);
      return true;
    }
    if (typeof $notification !== "undefined" && $notification &&
        typeof $notification.post === "function") {
      var options = url
        ? (PLATFORM === "loon" ? { openUrl: url } : { url: url }) : undefined;
      if (options) $notification.post(title, subtitle, message, options);
      else $notification.post(title, subtitle, message);
      return true;
    }
    return false;
  }
  function normalizeDoneValue(value) {
    if (!value || PLATFORM === "quantumult-x" || typeof value.status !== "string") {
      return value || {};
    }
    var matched = value.status.match(/\b(\d{3})\b/);
    if (matched) value.status = Number(matched[1]);
    return value;
  }
  ...
  function done(value) {
    if (!ended) { ended = true; $done(normalizeDoneValue(value)); }
  }
  function storageKey(key) {
    return NS + (/^(gallery|gallery-link|work|lock|hobby|known)$/.test(key) ? "all:" + ITEM + ":" : "") + key;
  }
  function drop(key) { storageRemove(storageKey(key)); }
  ...
  function read(key) {
    try { return JSON.parse(storageRead(storageKey(key)) || "null"); }
    catch (_) { return null; }
  }
  function save(key, value) {
    return storageWrite(JSON.stringify(value), storageKey(key));
  }
  function saveRaw(key, value) {
    return storageWrite(JSON.stringify(value), NS + key);
  }
  function readRaw(key) {
    try { return JSON.parse(storageRead(NS + key) || "null"); }
    catch (_) { return null; }
  }
  ...
  function removeGalleryImageMappings(gallery) {
    ...
    gallery.rows.forEach(function (row) {
      ...
      if (storageRemove(NS + "image:" + id)) removed++;
    });
    return removed;
  }
  function purgeTrackedAlbum(item, albumId, entry) {
    ...
    var removed = 0;
    try {
      var scoped = JSON.parse(storageRead(
    ...
    ["album-gallery:", "album-work:", "album-lock:", "native-pages:"].forEach(function (prefix) {
      if (storageRemove(NS + prefix + item + ":" + albumId)) removed++;
    });
    if (storageRemove(NS + "album-owner:" + albumId)) removed++;
    if (storageRemove(NS + "standalone-album:" + albumId)) removed++;
  ...
  function purgeTrackedItem(item, entry) {
    ...
    (Array.isArray(entry.routes) ? entry.routes : []).forEach(function (route) {
      if (storageRemove(NS + "exhobby-route:" + route)) removed++;
    });
    if (storageRemove(NS + "exhobby-route-for-item:" + item)) removed++;
    return removed;
  }
  ...
  function clearTrackedCaches() {
    ...
    storageRemove(NS + "cache-index");
    storageRemove(NS + "gc-last");
    return removed;
  }
    ...
    for (var probe = 0; probe < 1000; probe++) {
      ...
      if (!occupied) {
        if (savedRoute && savedRoute !== route) {
          storageRemove(NS + "exhobby-route:" + savedRoute);
        }
  ...
  async function injectGalleryIntoNativePictures(doc, ctx) {
    ...
    // If an unusually short earlier native page leaves a gap that cannot be
    // reconstructed without hidden requests, keep the current Hpoi page intact
    // instead of replacing it with an empty list.
    if (!exRows.length && !visibleNative.length && nativeRows.length) {
      log("lazy scoped album fallback itemId=" + albumItemId +
        " page=" + page + " nativeGapAt=" + nativeStart);
      return false;
    }
  ...
  async function notifyVerification(error) {
    ...
    if (/^https:\/\/[^\s/?#]+\/[^\s/?#]+\/?$/.test(barkURL)) {
      ...
      try {
        ...
        if (responseStatus(response) === 200) return;
      } catch (_) {} finally {
        if (typeof clearTimeout === "function") clearTimeout(timer);
      }
      log("v3.7.0 Bark delivery failed; falling back to local notice");
    }
    postNotification(title, "打开 EXHOBBY 验证", message + "\n" + url, url);
  }
  function copy(value) {
  ...
  function paramsFor(source) {
    source = source || req;
    var out = pairs(String(source.url || "").split("?")[1]);
    var body = source.body || "", fields = {};
    if (/^\s*\{/.test(body)) {
  ...
  function params() { return paramsFor(req); }
  function correlationSession(source, publicFields) {
    source = source || req;
    if (source.sessionIndex != null) return "qx:" + String(source.sessionIndex);
    if (source.id != null && String(source.id)) return "request-id:" + String(source.id);
    var parts = [endpoint || "unknown", String(source.method || "GET").toUpperCase(),
      String(source.url || "").split("?")[0]];
    FIELDS.forEach(function (key) {
      if (publicFields && publicFields[key] != null) {
        parts.push(key + "=" + String(publicFields[key]));
      }
    });
    return "fingerprint:" + hashText(parts.join("|"));
  }
  function requestContextKey(session) {
    return "request:" + (parseInt(hashText(session), 16) % 1024);
  }
  function requestAfterChanges(output) {
    var source = {
      url: output && output.url != null ? output.url : req.url,
      body: output && output.body != null ? output.body : req.body,
      method: req.method,
      headers: req.headers
    };
    if (req.sessionIndex != null) source.sessionIndex = req.sessionIndex;
    if (req.id != null) source.id = req.id;
    return source;
  }
  ...
  function virtualItem(p) {
    for (var i = 0; i < ID_FIELDS.length; i++) {
      ...
      var standalone = readRaw("standalone-album:" + n);
      if (standalone && validItem(Number(standalone.item)) &&
          Number(standalone.itemId) === n) {
        return { type: "standalone-owner", item: Number(standalone.item),
          route: n, nativeId: Number(standalone.nativeId) || 0 };
      }
  ...
  function captureExhobbySession() {
    ...
    if (typeof $response === "undefined" || responseStatus($response) !== 200) {
      done(); return true;
    }
    ...
    if (confirmed) {
      ...
      if (cookie && !/[\r\n]/.test(cookie) && !/[\r\n]/.test(agent)) {
        if (save("browser-session", { cookie: cookie, agent: agent, time: Date.now() })) {
          ...
          log("v3.7.0 browser session saved; reopen Hpoi");
        } else log("v3.7.0 browser session cache write failed");
      } else log("v3.7.0 gallery loaded, but Cookie header missing; tap More in Safari");
    }
    done();
    return true;
  }
  ...
  function makeExClient(options) {
    ...
    function htmlInfo(html, stage) {
      ...
      log("v3.7.0 " + stage + " title=" + label(title && title[1]) +
    ...
    function readText(response, stage) {
      ...
      log("v3.7.0 " + stage + " HTTP=" + response.statusCode +
    ...
    async function request(url, method, body, stage, ticket) {
      ...
      for (var hop = 0; hop < 5; hop++) {
        ...
        var response;
        try { response = await httpFetch(options); }
        ...
        if ([301, 302, 303, 307, 308].indexOf(status) >= 0) {
          ...
          log("v3.7.0 " + stage + " redirect=" + url.split("?")[0]);
    ...
    function parseFirst(html) {
      ...
      log("v3.7.0 " + scopeLabel + " first HTML count=" + rows.length);
      return rows;
    }
    async function bootstrap(ticket) {
      ...
      log("v3.7.0 " + scopeLabel + " fresh map.url ready; preview list not used");
    ...
    return {
      page: async function (page, ticket) {
        ...
        log("v3.7.0 " + scopeLabel + " page=" + page + " count=" + list.length);
        return list;
      }
    };
  }
  function fetchClientPage(client, page, milliseconds, label) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        finish(new Error("v3.7.0 " + (label || "gallery") +
  ...
  async function loadAlbumGallery(albumItemId) {
    ...
    try {
      ...
      if (current && current.owner === owner) storageRemove(NS + lockKey);
    }
  }
  ...
  async function loadGallery() {
    ...
    try {
      ...
      if (work.next > 1 && work.first !== firstSignature) {
        work = { version: 30, started: now, next: 1, rows: [], last: "", first: "" };
        log("v3.7.0 first page changed; restarting pagination");
      }
      work.first = firstSignature;
  ...
  async function main() {
    ...
    var p = params();
    var session = correlationSession(req, p);
    var key = requestContextKey(session);
    if (!isResponse) {
      var originalSession = session;
      var v = endpoint === "hobby/album" ? null : virtualItem(p);
      log("runtime=v3.7.0 platform=" + PLATFORM + " phase=request endpoint=" + endpoint);
      ...
      var ctx = { time: Date.now(), endpoint: endpoint, p: p, v: v,
        target: ITEM };
      var output = {};
      if (v && (v.type === "normal-owner" || v.type === "standalone-owner")) {
        ctx.remapped = false;
        log("v3.7.0 " + (v.type === "standalone-owner" ? "standalone" : "native") +
        ...
        ctx.remapped = true;
        log("v3.7.0 native album navigation itemId restored " +
          v.proxy + "->" + v.route);
        ...
        ctx.remapped = true;
        log("v3.7.0 normal album proxy remapped to native route");
      } else if (v) {
        ...
        ID_FIELDS.forEach(function (k) {
          ...
          if (p[k] && (nativeId === ALBUM || nativeId > PIC_BASE || dedicatedRoute) &&
            ...
            changes[k] = safeId;
            log("v3.7.0 remap request field " + k + " via template ID");
          }
        });
        output = changeRequest(changes);
        ctx.remapped = true;
      }
      var responseRequest = requestAfterChanges(output);
      var responseSession = correlationSession(responseRequest, paramsFor(responseRequest));
      ctx.sessions = [originalSession];
      if (ctx.sessions.indexOf(responseSession) < 0) ctx.sessions.push(responseSession);
      for (var contextIndex = 0; contextIndex < ctx.sessions.length; contextIndex++) {
        if (!save(requestContextKey(ctx.sessions[contextIndex]), ctx)) {
          throw new Error("request context write failed");
        }
      }
      return done(output);
    }
    ...
    var ctx = read(key);
    if (!ctx || !Array.isArray(ctx.sessions) || ctx.sessions.indexOf(session) < 0 ||
        ctx.endpoint !== endpoint ||
        Date.now() - ctx.time > 60000) ctx = null;
    else ctx.sessions.forEach(function (storedSession) {
      storageRemove(NS + requestContextKey(storedSession));
    });
    if (ctx && ctx.target) setTarget(ctx.target);
    var doc;
    try { doc = JSON.parse($response.body); } catch (_) { return done(); }
    if (ctx && ctx.v) {
      if (ctx.v.type === "normal-owner" || ctx.v.type === "standalone-owner") {
    ...
    if (endpoint === "album/detail" && doc.data.album) {
      ...
      var standaloneItemId = Number(album.itemId);
      var standaloneNativeId = Number(album.id);
      if (Number.isInteger(standaloneItemId) && standaloneItemId > 0 &&
          standaloneItemId < ALBUM_BASE &&
          Number.isInteger(standaloneNativeId) && standaloneNativeId > 0 &&
          standaloneNativeId < ALBUM_BASE) {
        saveRaw("standalone-album:" + standaloneItemId, {
          item: standaloneNativeId,
          itemId: standaloneItemId,
          nativeId: standaloneNativeId,
          time: Date.now()
        });
        ITEM = standaloneNativeId;
        trackCacheValue("albums", standaloneItemId);
        log("standalone album registered id=" + standaloneNativeId +
          " itemId=" + standaloneItemId);
      }
    ...
    if (endpoint === "hobby/album" && ITEM && Number(ctx.p.id) === ITEM &&
      ...
      // v3.7.0: native Hpoi albums keep their original id/itemId unchanged.
  ...
  main().catch(function (e) {
    log("v3.7.0 " + (e && e.message ? e.message : "operation failed"));
